...
fun <T> Set<T>.updateItems(predicate: (T) -> Boolean, reducer: T.() -> T): Set<T> {
    return mapTo(HashSet()) { item ->
        if (predicate(item)) item.reducer() else item
    }
}
...
/**
 * Convert an untyped array to a typed list.
 */
inline fun <reified T> Array<*>.toTypedList(): List<T> {
    val destination = ArrayList<T>(size)
    forEach {
        if (it is T) {
            destination.add(it)
