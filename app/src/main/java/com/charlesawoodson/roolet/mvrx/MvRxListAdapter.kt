...
/**
 * The [MvRxListAdapter] serves as the base adapter for all RecyclerView based lists. The adapter is designed
 * to be created as a field within fragments and then attached to a RecyclerView when the fragment's view has been
 * created.
 *
 * To construct an instance of the [MvRxListAdapter] a fragment must be provided so the lifecycle of the view holders
 ...
 * ```
 */
abstract class MvRxListAdapter<T : Any>(fragment: Fragment) : RecyclerView.Adapter<MvRxListViewHolder<out T>>() {
                ...
                fun onDestroyView() {
                    ...
                    if (recyclerView != null) {
                        ...
                        for (index in data.indices) {
                            ...
                            if (viewHolder != null) {
                                /*
                                 *  Prevent view holders that are currently on the screen from being put in the pool
                                 *  which would cause them to disappear from the screen while the exit animation is
                                 *  running.
                                 */
    ...
    @CallSuper
    override fun onBindViewHolder(holder: MvRxListViewHolder<out T>, position: Int) {
        @Suppress("UNCHECKED_CAST")
        (holder as MvRxListViewHolder<T>).bind(data[position])
    }

    @CallSuper
    override fun onBindViewHolder(holder: MvRxListViewHolder<out T>, position: Int, payloads: MutableList<Any>) {
        if (payloads.isEmpty()) {
            ...
            if (changeSet.isEmpty()) {
                ...
                @Suppress("UNCHECKED_CAST")
                (holder as MvRxListViewHolder<T>).bind(data[position], changeSet)
            }
        }
    }
    ...
    override fun onAttachedToRecyclerView(recyclerView: RecyclerView) {
        ...
        if (recyclerView.recycledViewPool !is MvRxRecycledViewPool) {
            recyclerView.setRecycledViewPool(MvRxRecycledViewPool())
        }
    }
    ...
    @CallSuper
    override fun onViewAttachedToWindow(holder: MvRxListViewHolder<out T>) {
        super.onViewAttachedToWindow(holder)
        holder.attach()
    }

    @CallSuper
    override fun onViewDetachedFromWindow(holder: MvRxListViewHolder<out T>) {
        super.onViewDetachedFromWindow(holder)
        holder.detach()
    }

    @CallSuper
    override fun onViewRecycled(holder: MvRxListViewHolder<out T>) {
        super.onViewRecycled(holder)
        holder.recycle()
    }

    final override fun onFailedToRecycleView(holder: MvRxListViewHolder<out T>): Boolean {
